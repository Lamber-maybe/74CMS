<template>
  <div class="message-list" :class="$store.state.LoginType==1?'com':''" id="J_message_list" :style="'height:'+messagelist_height">
      <div class="loading" v-if="loading">
        <div></div>
        <div></div>
        <div></div>
      </div>
      <div class="more" v-else-if="finish===false" @click="loadMore">查看更多消息</div>
      <MessageItem @showHellomsg="showHellomsg" :chat_starttime="chat_starttime" @applyJob="applyJob" @refuseWechat="refuseWechat" @agreeWechat="agreeWechat" @refuseMobile="refuseMobile" @agreeMobile="agreeMobile" :dataset="messageList" />
  </div>       
</template>
...
export default {
  ...
  methods:{
    ...
    getMessageList(chat_id) {
      this.chat_id = chat_id
      http
        .post(api.im_message_list, {token:this.$store.state.imToken, chat_id:chat_id,page:this.page })
        .then(res => {
          if (res.data.items.length == 0) {
            this.finish = true;
            this.loading = false;
          }else{
            let res_list = res.data.items
            this.messageList = res_list.concat(this.messageList)
            this.page++
            this.loading = false
            let prevHeight = document.getElementById("J_message_list").scrollHeight
            this.$nextTick(() => {

<template>
  <div>
    <div class="cc_top">
      <input type="text" class="cip absolute_for_y" v-model="keyword" placeholder="请输入企业名称或手机号" />
    ...
    >
      <van-radio-group v-model="selected_id">
        <div class="cc_item" v-for="(item,index) in dataset" :key="index" @click="funSelected(item)">
          ...
          <van-radio class="absolute_for_y" :name="item.uid"></van-radio>
        </div>
      </van-radio-group>
    </van-list>
    </div>
  ...
  export default {
    ...
    methods:{
      ...
      fetchData (init) {
        ...
        http
          .get(api.searchCompany, conditions)
          .then(res => {
            if (init === true) {

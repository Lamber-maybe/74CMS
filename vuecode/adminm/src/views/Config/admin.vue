<template>
  <div class="body">
    ...
    <van-cell-group v-else>
      <van-cell>
        <template #title>
          <span class="cus_title">用户名：</span
          ><span class="cus_content">{{ info.username }}</span>
          <van-button
            round
            plain
            type="info"
            class="cus_btn absolute_for_y"
      ...
      <van-cell>
        <template #title>
          <span class="cus_title">所属角色：</span
          ><span class="cus_content">{{ info.role_name }}</span>
        </template>
      </van-cell>
      <van-cell>
        <template #title>
          <span class="cus_title">创建时间：</span
          ><span class="cus_content">{{ info.addtime | timeFilter }}</span>
        </template>
      </van-cell>
      <van-cell>
        <template #title>
          <span class="cus_title">最近登录：</span
          ><span class="cus_content">{{
            info.last_login_time | timeFilter
          }}</span>
        </template>
      </van-cell>
      <van-cell>
        <template #title>
          <span class="cus_title">登录IP：</span
          ><span class="cus_content">{{ info.last_login_ip }}</span>
        </template>
      </van-cell>
      <van-cell>
        <template #title>
          <span class="cus_title">最近登录：</span
          ><span class="cus_content">{{ info.last_login_ipaddress }}</span>
        </template>
      </van-cell>
      <van-cell>
        <template #title>
          <span class="cus_title">登录密码：</span
          ><span class="cus_content">******</span>
          <van-button
            round
            plain
            type="info"
    ...
    <div class="bottom">
      <van-button type="primary" block color="#1787fb" @click="logout"
        >退出登录</van-button
      >
    </div>
  </div>

<template>
  <div>
    ...
    >
    <div class="b1">
      <div class="b_item" @click="$router.push('/setmeal/'+item.uid+'?companyname='+item.companyname)" v-for="(item,index) in dataset" :key="index">
        <p class="t1 substring">{{item.companyname}}</p><van-icon class="arrow" name="arrow" color="#999999" />
        <div class="i_tag wait" v-if="item.expire==1">已过期</div>
        <div class="t_line">
          <div class="tl_cell"><span class="l_title">套餐：</span><span class="l_con">{{item.setmeal_name}}</span></div>
          <div class="tl_cell"><span class="l_title">到期：</span><span class="l_con">{{item.deadline_cn}}</span></div>
        </div>
        <div class="t_line last">
          <div class="tl_cell"><span class="l_title">简历点：</span><span class="l_con">{{item.download_resume_point}}</span></div>
          <div class="tl_cell"><span class="l_title">剩余天数：</span><span class="l_con">{{item.surplus_days}}</span></div>
        </div>
        <div class="i_btn b1 blue">编辑</div>
      </div>
    </div>
    </van-list>
  </div>
  ...
  export default {
    ...
    methods: {
      ...
      fetchData (init) {
        ...
        http
          .get(api.companySetmealList, conditions)
          .then(res => {
            if (init === true) {

  ...
  <div>
    ...
    <div class="b1">
      <div class="b_item" v-for="(item,index) in dataset" :key="index">
        <p class="t1 substring">{{item.type_cn}}</p>
        ...
        <div class="t_line">
          ...
          <div class="tl_cell"><span class="l_title">提交：</span><span class="l_con">{{item.addtime|timeFilter}}</span></div>
        </div>
        <div class="t_line last">
          <div class="tl_cell lh">
            <span class="l_title">内容：</span>
            <span class="l_con">{{item.content}}</span>
          </div>
        </div>
        <div class="i_btn b1 blue" @click="funHandler(item)">处理</div><div class="i_btn b2" @click="funDel(item)">删除</div>
      </div>
    </div>
    </van-list>
    <van-action-sheet
  ...
  export default {
    ...
    data () {
      return {
        ...
        page: 1,
        pagesize: 15,
        options_status:['未处理','已处理'],
        handlerActions:[{name:'已处理',id:1},{name:'未处理',id:0}],
        handlerRow:{}
      }
    },
    created () {
    ...
    methods: {
      ...
      fetchData (init) {
        ...
        http
          .get(api.feedbackList, conditions)
          .then(res => {
            if (init === true) {
      ...
      funDel(row){
        this.$dialog
          ...
          .then(() => {
            http
            .post(api.feedbackDelete, {id:[row.id]})
            .then(res => {
      ...
      doHandler(action){
        http
          .post(api.feedbackHandler, {id:[this.handlerRow.id],status:action.id})
          .then(res => {
            this.$toast.success(res.message)
            this.fetchData(true)

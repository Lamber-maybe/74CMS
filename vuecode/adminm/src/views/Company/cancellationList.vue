    ...
    <div class="b1">
      <div class="b_item" v-for="(item,index) in dataset" :key="index">
        ...
        <div class="t_line">
          ...
          <div class="tl_cell"><span class="l_title">申请时间：</span><span class="l_con">{{item.addtime|timeFilter}}</span></div>
        </div>
        <div class="t_line last">
          <div class="tl_cell"><span class="l_title">手机号：</span><span class="l_con">{{item.mobile}}</span></div>
          <div class="tl_cell"><span class="l_title">处理时间：</span>
          <span class="l_con" v-if="item.handlertime>0">{{item.handlertime|timeFilter}}</span>
          <span class="l_con" v-else>-</span>
          </div>
        </div>
        <div class="i_btn b1" @click="funDel(item)">删除</div><div class="i_btn b2" @click="funHandle(item)">处理</div>
      </div>
    </div>
    </van-list>
  </div>
  ...
  .b1 {
    .b_item {
      ...
      .i_btn {
        ...
        &.gray { color: #999999;border-color: #ececec; }
      }
      .i_tag {

  ...
  <div>
    <el-card class="box-card" style="position:relative;">
      <el-date-picker
        ...
        @change="fetchData"
      />
      <el-select v-model="platform" size="mini" placeholder="请选择销售人员" style="position:absolute;right:268px;top:18px;z-index:1;width:120px;" @change="fetchData">
        <el-option label="销售人员" value="" />
        <el-option v-for="(item, index) in platformOptions" :key="index" :label="item.name" :value="item.id" />
      </el-select>
      <div id="work_life_cycle" :style="'height: ' + height + ';'" />
    </el-card>
  </div>
</template>

<script>
import { workLifeCycle } from '@/api/company_crm';
...
export default {
  ...
  mounted() {
    this.$nextTick(function() {
      this.drawChart('work_life_cycle');
    });
  },
  created() {
  ...
  methods: {
    fetchData(e, init = false) {
      ...
      workLifeCycle(params).then(response => {

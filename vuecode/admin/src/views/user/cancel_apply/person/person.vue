  ...
  <div class="app-container">
    <el-card class="box-card">
      ...
      <div style="float:right;z-index: 1;position: relative;margin-top: 13px;">
        <span class="switch" @click="goto('/user/person_cancel_backup')">
          ...
          >
            <template slot="header" slot-scope="scope">
              ...
              <div :class="items.is_sortable == 'custom' ? 'sotrTime' : ''" @mouseenter="locks(items.field)" @mouseleave="locks(items.field)">
                ...
                <div v-if="items.is_lock_display" slot="reference" class="unlock_display" @click="locking(items.field)"></div>
                <div v-else slot="reference" class="unlock" @click="locking(items.field)" ></div>
              </div>
            </template>
          ...
          >
            <template slot="header" slot-scope="scope">
              <div :class="items.is_sortable == 'custom' ? 'sotrTime' : ''" @mouseenter="locks(items.field)" @mouseleave="locks(items.field)">
                ...
                <div v-if="items.is_lock_display" slot="reference" class="lock_display" @click="locking(items.field)" ></div>
                <div v-else slot="reference" class="lock" @click="locking(items.field)" ></div>
              </div>
            </template>
            <template slot-scope="scope">
              <div v-if="items.field == 'id'">
  ...
  data() {
    return {
      ...
      fieldData: [
        ...
        { name: '简历名称', field: 'contact', select: true, is_locking: false, is_lock_display: false, width: 245, is_sortable: false },
        { name: '注册时间', field: 'regtime', select: true, is_locking: false, is_lock_display: false, width: 245, is_sortable: false },
        { name: '手机号', field: 'mobile', select: true, is_locking: false, is_lock_display: false, width: 245, is_sortable: false },
        { name: '处理状态', field: 'status', select: true, is_locking: false, is_lock_display: false, width: 175, is_sortable: false },
        { name: '申请时间', field: 'addtime', select: true, is_locking: false, is_lock_display: false, width: 245, is_sortable: 'custom' },
        { name: '处理时间', field: 'handlertime', select: true, is_locking: false, is_lock_display: false, width: 245, is_sortable: 'custom' }
      ],
      auditScreen: '',
      processingStatus: '',
      sortType: '',
  ...
  methods: {
    ...
    funHandle(row){
      ...
      if (row.is_backups === 1) {
        ...
        that
          .$confirm('您在注销之前，可备份保留会员信息，是否备份' + row.contact + '的会员信息？', '提示', {

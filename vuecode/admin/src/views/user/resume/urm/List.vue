  ...
  <div class="app-container">
    <el-card class="box-card">
      ...
      <div style="float:right;z-index: 8;position: relative;">
        <el-button size="small" type="primary" @click="goto('/user/resume/add')">
          新增简历
        </el-button>
      </div>
      ...
      <div style="position: relative;">
        <div
          :class="educationFilter !='' ||
            ...
            auditFilter !='' ||
            worksFilter !='' ||
            contactStatusFilter !='' || citycategoryFilter !=''? 'setField_s' : 'setField'"
        >
          <el-popover
            v-model="visiblepop"
            ...
            <template slot-scope="scope">
              ...
              <div v-if="items.field == 'member_mobile'">
                ...
                <span v-else>
                  ...
                  <div class="dial" title="拨打会员手机号">
                    <div class="disal_img" @click="clickDial(scope.row.member_mobile, scope.row.fullname )" />
                  </div>
                </span>
              </div>
      ...
      <div class="bortton-page">
        <el-col :span="10">
          ...
          <el-button size="small" type="primary" @click="funLevelBatch('all')">
            简历等级
          </el-button>
...
export default {
  ...
  data(){
    return {
      ...
      fieldData: [
        ...
        { 'name': '最后跟进', 'field': 'final_follow', 'select': true, 'is_locking': false, 'is_lock_display': false, 'width': 160, 'is_sortable': false },
  ...
  methods: {
    ...
    classify(){
      classify({ 'type': 'education,resumeAudit,citycategory,jobcategory' }).then(res => {
        ...
        for (var i = 0; i <= this.educationData.length - 1; i++){
          if (this.educationScreen == this.educationData[i].id){
            this.educationFilter = {
              'field': 'education_cn', 'name': this.educationData[i].name
            }
          }
        }
      } else {
        ...
        for (var i = 0; i <= this.experienceData.length - 1; i++){
          if (this.experienceScreen === this.experienceData[i].id){
            this.experienceFilter = {
              'field': 'experience_cn', 'name': this.experienceData[i].name
            }
          }
        }
      } else {
        ...
        for (var i = 0; i <= this.highQualityData.length - 1; i++){
          if (this.highQualityScreen === this.highQualityData[i].id){
            this.highQualityFilter = {
              'field': 'high_quality', 'name': this.highQualityData[i].name
            }
          }
        }
      } else {
        ...
        for (var i = 0; i <= this.deliveryNumData.length - 1; i++){
          if (this.deliveryNumScreen == this.deliveryNumData[i].id){
            this.deliveryNumFilter = {
              'field': 'delivery_num', 'name': this.deliveryNumData[i].name
            }
          }
        }
      } else {
        ...
        for (var i = 0; i <= this.downloadedData.length - 1; i++){
          if (this.downloadedScreen == this.downloadedData[i].id){
            this.downloadedFilter = {
              'field': 'downloaded', 'name': this.downloadedData[i].name
            }
          }
        }
      } else {
      ...
      if (this.viewedScreen != ''){
        for (var i = 0; i <= this.viewedData.length - 1; i++){
          if (this.viewedScreen == this.viewedData[i].id){
            this.viewedFilter = {
              'field': 'viewed', 'name': this.viewedData[i].name
            }
          }
        }
      } else {
        ...
        for (var i = 0; i <= this.invitationData.length - 1; i++){
          if (this.invitationScreen == this.invitationData[i].id){
            this.invitationFilter = {
              'field': 'invitation', 'name': this.invitationData[i].name
            }
          }
        }
      } else {
      ...
      if (this.auditScreen !== ''){
        for (var i = 0; i <= this.auditData.length - 1; i++){
          if (this.auditScreen === this.auditData[i].id){
            this.auditFilter = {
              'field': 'audit', 'name': this.auditData[i].name
            }
          }
        }
      } else {
      ...
      if (this.worksScreen != ''){
        for (var i = 0; i <= this.worksData.length - 1; i++){
          if (this.worksScreen == this.worksData[i].id){
            this.worksFilter = {
              'field': 'works', 'name': this.worksData[i].name
            }
          }
        }
      } else {
        ...
        for (var i = 0; i <= this.contactStatusData.length - 1; i++){
          if (this.contactStatusScreen == this.contactStatusData[i].id){
            this.contactStatusFilter = {
              'field': 'contact_status', 'name': this.contactStatusData[i].name
            }
          }
        }
      } else {

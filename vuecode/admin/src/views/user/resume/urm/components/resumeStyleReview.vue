<template>
  <div>
          ...
          <div v-for="item in arr" class="check-item">
            <el-checkbox :label="item.id">
              <el-tooltip class="item" effect="dark" placement="top">
                <div slot="content">{{ item.fullname }}</div>
                <div class="com-name substring">{{ item.fullname }}</div>
              </el-tooltip>
            </el-checkbox>
            <div class="com-img">
    ...
    <!-- //审核弹窗 -->
    <el-dialog title="将所选简历作品设置为" :visible.sync="dialogIsDispay" width="420px">
      ...
      <div slot="footer" class="dialog-footer">
        ...
        <el-button size="medium" type="primary" :loading="loading" @click="toDo('sure', '')">确定</el-button>
      </div>
    </el-dialog>
    <!-- 修改简历作品弹窗 -->
    <el-dialog title="修改简历作品" :visible.sync="dialogIsEdit" width="375px">
      <div class="isDispayBox label-box">
        <span class="label-txt">简历：</span>
        <div>{{ resume_info.fullname }}</div>
      </div>
      <div class="isDispayBox label-box">
        ...
        <el-upload class="logo-uploader" :action="apiUpload" :headers="headers" :show-file-list="false" :on-success="handleSuccess" :before-upload="beforeUpload">
          ...
          <img v-if="styleImg" :src="resume_info.img_src" class="logo">
      ...
      <div slot="footer" class="dialog-footer">
        ...
        <el-button size="medium" type="primary" :loading="editLoading" @click="toDo('editSure', '')">确定</el-button>
      </div>
    </el-dialog>
  </div>
</template>
...
import { allCustomersImg, resumeImgAudit, resumeImgDelete, resumeImgEdit } from '@/api/resume_urm'
export default {
  ...
  methods: {
    ...
    deleteOprate() {
      this.$confirm('确定删除 ' + this.resume_info.fullname + ' 简历作品吗？', '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      })
        .then(() => {
          resumeImgDelete({ id: this.r_id }).then(res => {
            if (res.code == 200) {
    ...
    exmineOprate() {
      this.loading = true
      const params = {
        id: this.is_all || this.checkedCities.length > 0 ? this.checkedCities : this.r_id,
        audit: parseInt(this.is_audit)
      }
      resumeImgAudit(params).then(res => {
        if (res.code == 200) {
          this.loading = false

    ...
    <el-card class="box-card">
      ...
      >
        <el-button
          ...
          round
          @click="funAdd"
        ><i class="el-icon-plus" /> 你还没有添加语言能力，点击添加</el-button>
      </div>
      <div
        ...
        class="item-row"
      >
        <span class="item-li">语种：{{ item.language_cn }}</span>
        <span class="item-li">熟悉程度：{{ item.level_cn }}</span>
...
<script>
import diaform from './resumeLanguageForm.vue'
import { resumeLanguageList, resumeLanguageDelete } from '@/api/resume'
    ...
    funDelete(row) {
      ...
      that.$confirm('此操作将永久删除该数据, 是否继续?', '提示', {
        ...
        .then(() => {
          ...
          resumeLanguageDelete(param).then(response => {

    ...
    <el-card class="box-card">
      ...
      >
        ...
        <el-form-item label="">
          ...
          <el-button @click="goto('/shortvideo/ad')">返回</el-button>
        </el-form-item>
      </el-form>
    </el-card>
  </div>
...
<script>
import { categoryEdit, adPlatformList } from '@/api/shortvideo'
...
export default {
  data() {
    var validateAlias = (rule, value, callback) => {
      ...
      if (this.is_sys == 0 && value.startsWith('QS_')) {
    ...
    fetchInfo() {
      ...
      adPlatformList()
        .then(response => {
          ...
          return categoryEdit({ id: this.$route.query.id }, 'get')
        })
        .then(response => {
          this.form = { ...response.data.info }
          this.is_sys = response.data.info.is_sys
        ...
        if (valid) {
          categoryEdit(insertData)
            .then(response => {
              ...
              setTimeout(function() {
                that.$router.push('/shortvideo/ad')
              }, 1500)
              return true
            })
            .catch(() => {})

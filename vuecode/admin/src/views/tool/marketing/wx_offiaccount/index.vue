<template>
  <div class="app-container">
    <addTemplate v-if="templateFlag" :template-id="templateId" :template-type="templateType" />
    ...
    <el-tabs v-else v-model="tabActive" type="border-card">
      <el-tab-pane label="公众号营销" name="1">
        ...
        <div v-if="tabActive == 1" class="el-row--flex">
          <el-form v-show="chooseDone===false" class="common-form" label-width="100px" style="width: 60%">
            <el-form-item label="信息类型" style="margin-bottom:20px;">
              <el-radio-group v-model="data.type" @change="radioChange">
                <el-radio label="job_list">
                  职位列表
                </el-radio>
                <el-radio label="company_list">
                  企业列表
                </el-radio>
                <!--<el-radio label="resumelist">-->
                <!--简历列表-->
                <!--</el-radio>-->
                <el-radio label="appoint_company">
                  指定企业
                </el-radio>
              </el-radio-group>
            </el-form-item>
          ...
          <div class="rightList">
            ...
            <div v-if="datalist.length == 0" class="autos emptyImg">
              <img src="../../../../assets/images/empty.jpg" alt="">
              <span class="words">暂无数据，请点击搜索并添加</span>
            </div>
            <el-checkbox-group v-else v-model="checkList" @change="checkChange">
              <draggable v-model="datalist" class="autos" :options="{animation:200}" @update="datadragEnd">
                <div v-for="(item,index) of datalist" :key="index" class="dataCard">
                  <el-checkbox :label="JSON.stringify(item)" class="checkBox">
                    <div class="left">
                      <div class="inner">
                        <span class="title">{{ data.type == 'job_list' ? item.job_name : item.company_name }}</span>
                        <div v-if="data.type == 'job_list'" class="content">{{ item.company_name ? item.company_name : '公司异常' }}</div>
                        <div v-else class="content">
                          <span v-if="item.job_list.length > 0"><b>{{ item.job_list[0].job_name }}</b>等{{ item.job_list.length }}个职位</span>
                          <span v-else>暂无发布职位</span>
                        </div>
                      </div>
                      <span>ID：{{ data.type == 'job_list' ? item.job_id : item.company_id }}</span>
                    </div>
                  </el-checkbox>
            ...
            <div class="rightBottom">
              <div>
                ...
                >全选</el-checkbox>
                <span style="margin-left: 16px;font-size: 14px;color: #999999;">当前已选{{ checkList.length }}条</span>
              </div>
              <div>
                <el-button size="small" type="danger" @click="delClick">删除</el-button>
                <el-button size="small" type="primary" @click="releaseClick">发布预览</el-button>
              </div>
            </div>
          </div>
        </div>
      ...
      >
        ...
        <div class="popup_box">
          <el-card shadow="never" :body-style="{padding:'10px 20px 20px'}">
            ...
            <el-radio-group v-model="radio" class="radioGroup" size="small" @change="templateRadioChange">
              <el-radio
          ...
          <el-card shadow="never" class="top10">
            ...
            <el-card shadow="never" class="top10" :body-style="{padding:'10px 20px'}" style="background: #f7f8f9;font-size: 14px">
              推荐复制内容后，在微信后台编辑器或135等排版编辑器中使用或二次编辑
            </el-card>
            <el-card shadow="never" class="top10" :body-style="{padding:'10px 20px'}" style="background: #fdf6ec;color: #e6a23c;font-size: 14px">
              部分模板存在图片资源，建议关闭微信后台设置中的【图片水印】功能
            </el-card>

            <el-button type="primary" class="top34" plain @click="copy">一键复制</el-button>
          </el-card>
          <!-- 关闭按钮 -->
  ...
  methods: {
    ...
    bodyObjStr(str, obj, items){
      ...
      for (let i = 0; i < items.length; i++){
        ...
        for (let j = 0; j < obj.length; j++){
          ...
          if (obj[j].label == '职位福利' && bodyStr.indexOf('{职位福利}') != -1){
            const arr = items[i][`${obj[j].value}`]
            let arrStr = ''
            if (arr){
              arrStr = arr.join(',')
            }
            bodyStr = bodyStr.replace(/{职位福利开始}{职位福利}{职位福利结束}/g, arrStr)
          } else if (obj[j].label == '企业福利' && bodyStr.indexOf('{企业福利}') != -1){
          ...
          } else if (obj[j].label == '企业职位' && bodyStr.indexOf('{职位列表开始}') != -1){
            ...
            if (arr){
              ...
              const newStr = (bodyStr.match(/({职位列表开始}(.+?){职位列表结束})/g))[0]
              bodyStr = bodyStr.replace(newStr, newJob)
            }
          } else if (obj[j].label == '企业H5场景码' && bodyStr.indexOf('企业H5场景码') != -1){
            const src = items[i][`${obj[j].value}`]
            if (src){
              const reg = /{企业H5场景码\|样式=&quot;(.+?)&quot;}/g
              bodyStr = this.matchStr(reg, bodyStr, src)
            }
          } else if (obj[j].label == '职位H5场景码' && bodyStr.indexOf('职位H5场景码') != -1){
            const src = items[i][`${obj[j].value}`]
            if (src){
            ...
            bodyStr = bodyStr.replace(/{小程序APPID}/g, this.common.mini_program_appid)
            ...
            bodyStr = bodyStr.replace(/{网站名称}/g, this.common.site_name || '')
          } else if (obj[j].label == '网站地址'){
            bodyStr = bodyStr.replace(/{网站地址}/g, this.common.site_domain || '')
          } else if (obj[j].label == '当期日期'){
            bodyStr = bodyStr.replace(/{当期日期}/g, parseTime(this.common.date, '{y}-{m}-{d}') || '')
          } else if (obj[j].label == '职位描述'){
            const key = items[i][`${obj[j].value}`]
            if (key){
              bodyStr = bodyStr.replace(eval(reg), '<p style="white-space: pre-wrap">' + key + '</p>')
            }
    ...
    delClick(){
      if (this.checkList.length == 0){
        ...
        this.$confirm('此操作将删除选中信息,如需恢复数据，请点击生成按钮，是否继续?', '提示', {
          ...
          .then(() => {
            ...
            let arr = []
            if (this.data.type == 'job_list'){
              arr = this.datalist.filter((item) => !checkList.some((ele) => ele.job_id === item.job_id))
            } else {
              arr = this.datalist.filter((item) => !checkList.some((ele) => ele.company_id === item.company_id))
            }
            this.datalist = arr
            this.checkList = []
            this.checkAll = false
    ...
    sortList(list){
      ...
      list.sort(function(a, b){
        if (that.data.type == 'job_list'){
          return that.datalist.map(s => s.job_id).indexOf(JSON.parse(a).job_id) < that.datalist.map(s => s.job_id).indexOf(JSON.parse(b).job_id) ? -1 : 1
        } else {
          return that.datalist.map(s => s.company_id).indexOf(JSON.parse(a).company_id) < that.datalist.map(s => s.company_id).indexOf(JSON.parse(b).company_id) ? -1 : 1
        }
      })
      return list
    },
    ...
    previewFun(){
      ...
      const newHead = this.headFootObjStr(headStr, this.headOption, this.common)
      const newTail = this.headFootObjStr(tailStr, this.tailOption, this.common)
      let newBody = ''
      if (this.data.type == 'job_list'){
    ...
    onSubmit() {
      const that = this
      if (that.data.type == 'appoint_company' && that.$refs.conditionData.form.company_ids.length === 0){
      ...
      marketingSearch(insertData)
        .then(response => {
          that.submiting = false
          that.datalist = response.data.items
          that.common = response.data.common
          that.checkList = []
          return true
        })
        .catch(() => {

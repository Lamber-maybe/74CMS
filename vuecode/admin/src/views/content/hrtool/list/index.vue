...
<script>
import { hrtoolList, hrtoolDelete } from '@/api/hrtool'
...
export default {
  ...
  methods: {
    fetchData() {
      ...
      hrtoolList(param).then(response => {
    ...
    funDelete(index, row) {
      ...
      that
        ...
        .then(() => {
          ...
          hrtoolDelete(param).then(response => {
    ...
    goTo(target) {
      this.$router.push('/content/hrtool/list/' + target)
    }
  }
}
</script>

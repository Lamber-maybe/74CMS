  ...
  <div class="app-container">
    <el-card class="box-card">
      ...
      >
        ...
        <el-table-column label="所属平台" min-width="100">
          <template slot-scope="scope">
            {{ scope.row.platform }}
          </template>
        </el-table-column>
        <el-table-column label="所属广告位" min-width="150">
          <template slot-scope="scope">
            {{ scope.row.cname }}
          </template>
        </el-table-column>
...
export default {
  ...
  methods: {
    ...
    fetchCategoryData() {
      getClassify({ type: 'adCategory' }).then(response => {
    ...
    goTo() {
      this.$router.push('/content/ad/list/add')
    }
  }
}
</script>

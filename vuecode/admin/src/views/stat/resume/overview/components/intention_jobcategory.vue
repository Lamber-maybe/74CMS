...
<script>
import { resumeOverviewIntentionJobcategory } from '@/api/stat'
...
export default {
  ...
  mounted() {
    this.$nextTick(function() {
      this.drawChart('intention_jobcategory')
    })
  },
  created() {
    this.fetchData()
  ...
  methods: {
    fetchData() {
      resumeOverviewIntentionJobcategory({}).then(response => {

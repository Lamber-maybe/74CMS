  ...
  <div class="app-container">
    ...
    >
      ...
      >
        ...
        >
          <el-input
            v-model.number="form.deduct_max"
            type="number"
          />
          元
        </el-form-item>
...
import {
  serviceJobStickList,
  serviceJobStickDelete,
  serviceJobStickAdd,
  serviceJobStickEdit
} from '@/api/service_job_stick'
...
export default {
  ...
  methods: {
    ...
    editSave(insertData, formName) {
      this.$refs[formName].validate(valid => {
        if (valid) {
          serviceJobStickEdit(insertData)
            .then(response => {
              this.$message.success(response.message)
              this.closeDialog()
              this.fetchData()

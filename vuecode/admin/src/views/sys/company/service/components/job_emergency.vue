...
<script>
import {
  serviceJobEmergencyList,
  serviceJobEmergencyDelete,
  serviceJobEmergencyAdd,
  serviceJobEmergencyEdit
} from '@/api/service_job_emergency'
  ...
  methods: {
    ...
    addSave(insertData, formName) {
      this.$refs[formName].validate(valid => {
        if (valid) {
          serviceJobEmergencyAdd(insertData)
            .then(response => {
    ...
    editSave(insertData, formName) {
      this.$refs[formName].validate(valid => {
        if (valid) {
          serviceJobEmergencyEdit(insertData)
            .then(response => {
              this.$message.success(response.message)
              this.closeDialog()
              this.fetchData()
    ...
    funDelete(index, row) {
      ...
      that
        ...
        .then(() => {
          ...
          serviceJobEmergencyDelete(param)
            .then(response => {

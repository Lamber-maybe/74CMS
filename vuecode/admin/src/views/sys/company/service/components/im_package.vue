<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column label="职聊次数" min-width="80">
    ...
    >
      ...
      >
        ...
        <el-form-item label="职聊次数" prop="times">
...
<script>
import {
  serviceImPackageList,
  serviceImPackageDelete,
  serviceImPackageAdd,
  serviceImPackageEdit
} from '@/api/service_im_package'
  ...
  methods: {
    ...
    addSave(insertData, formName) {
      this.$refs[formName].validate(valid => {
        if (valid) {
          serviceImPackageAdd(insertData)
            .then(response => {
    ...
    editSave(insertData, formName) {
      this.$refs[formName].validate(valid => {
        if (valid) {
          serviceImPackageEdit(insertData)
            .then(response => {
              this.$message.success(response.message)
              this.closeDialog()
              this.fetchData()

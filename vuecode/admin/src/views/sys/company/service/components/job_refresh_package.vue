<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column label="刷新次数" min-width="80">
    ...
    >
      ...
      >
        ...
        <el-form-item label="刷新次数" prop="times">
          <el-input v-model.number="form.times" type="number" />
        </el-form-item>
...
<script>
import {
  serviceJobRefreshPackageList,
  serviceJobRefreshPackageDelete,
  serviceJobRefreshPackageAdd,
  serviceJobRefreshPackageEdit
} from '@/api/service_job_refresh_package'
...
export default {
  ...
  methods: {
    fetchData() {
      ...
      const param = {}
      serviceJobRefreshPackageList(param)
        .then(response => {
    ...
    addSave(insertData, formName) {
      this.$refs[formName].validate(valid => {
        if (valid) {
          serviceJobRefreshPackageAdd(insertData)
            .then(response => {
    ...
    editSave(insertData, formName) {
      this.$refs[formName].validate(valid => {
        if (valid) {
          serviceJobRefreshPackageEdit(insertData)
            .then(response => {
              this.$message.success(response.message)
              this.closeDialog()
              this.fetchData()
    ...
    funDelete(index, row) {
      ...
      that
        ...
        .then(() => {
          ...
          serviceJobRefreshPackageDelete(param)
            .then(response => {

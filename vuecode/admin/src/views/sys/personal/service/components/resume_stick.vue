...
<script>
import {
  serviceResumeStickList,
  serviceResumeStickDelete,
  serviceResumeStickAdd,
  serviceResumeStickEdit
} from '@/api/service_resume_stick'
  ...
  methods: {
    ...
    addSave(insertData, formName) {
      this.$refs[formName].validate(valid => {
        if (valid) {
          serviceResumeStickAdd(insertData)
            .then(response => {
    ...
    editSave(insertData, formName) {
      this.$refs[formName].validate(valid => {
        if (valid) {
          serviceResumeStickEdit(insertData)
            .then(response => {
              this.$message.success(response.message)
              this.closeDialog()
              this.fetchData()
    ...
    funDelete(index, row) {
      ...
      that
        ...
        .then(() => {
          ...
          serviceResumeStickDelete(param)
            .then(response => {

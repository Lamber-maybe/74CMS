 ...
 *  将百度转为WGS-84:
 */
function transfromBaidutoWGS84(latitude, longitude){
  if (!isNaN(latitude) && !isNaN(longitude)){
    var lat = parseFloat(latitude)
    var lon = parseFloat(longitude)
    const res = transformFromBaiduToGCJ(lat, lon)
    const resWGS = transformFromGCJToWGS(res.latitude, res.longitude)
 ...
 *  将WGS-84转为百度:
 */
function transfromWGS84toBaidu(latitude, longitude){
  if (!isNaN(latitude) && !isNaN(longitude)){
    var lat = parseFloat(latitude)
    var lon = parseFloat(longitude)
    const res = transformFromWGSToGCJ(lat, lon)
    const resBaidu = transformFromGCJToBaidu(res.latitude, res.longitude)

<template>
    ...
    <div class="up-page-logo">
      <div class="logo-img">
        <img :src="basic.photo_img_src" border="0"/>
      </div>
    </div>
    <div class="up-sp-37"></div>
    <div class="up-txt-line font_3 font16 qs-center">{{basic.fullname}}</div>
    <div class="up-sp-45"></div>
    <div class="up-txt-line font_3 font14 qs-center">请点击下方区域上传您的 <span class="font_yellow">照片作品</span></div>
    ...
    <div class="up-txt-line font_3 font15 pd-left-45">已上传 <span class="font_yellow">照片作品</span> 图片：</div>
    <div class="up-sp-40"></div>
    <div class="img-callback-box">
      <div class="img-callback-cell" v-for="(item,index) in imgList" :key="index">
          <img :src="item.img_src">
          <div class="img-del" @click="handlerDel(item.id)"></div>
      </div>
      <div class="clear"></div>
    </div>
...
export default {
  ...
  methods: {
    fetchData () {
      http
        .get(api.editResume + '?user-token=' + this.access + '&platform=mobile')
        .then(res => {
    ...
    uploadImg (file) {
      ...
      this.uploading = true
      http
        .postFormData(api.resume_img_save + '?user-token=' + this.access + '&platform=mobile', { extra: 'resume_img', file: fileRaw })
        .then(res => {
    ...
    handlerDel (id) {
      this.$dialog
        ...
        .then(() => {
          // 确定按钮
          http
            .post(api.resume_img_delete + '?user-token=' + this.access + '&platform=mobile', {extra: 'resume_img', id: id})
            .then(res => {
              if (parseInt(res.code) === 200) {
                this.$notify({ type: 'success', message: res.message })
                this.imgList.splice(

...
export default {
  ...
  methods: {
    ...
    uploadImg (file) {
      ...
      this.uploading = true
      http
        .postFormData(api.company_img_upload + '?user-token=' + this.access + '&platform=mobile', { extra: 'company_img', file: fileRaw })
        .then(res => {
    ...
    handlerDel (id) {
      this.$dialog
        ...
        .then(() => {
          // 确定按钮
          http
            .post(api.company_img_del + '?user-token=' + this.access + '&platform=mobile', {extra: 'company_img', id: id})
            .then(res => {
              if (parseInt(res.code) === 200) {
                this.$notify({ type: 'success', message: res.message })
                this.imgList.splice(

...
import one from './company/one'
import two from './company/two'
import three from './company/three'
export default {
  name: 'ShareCompany',
  props: ['info', 'tpl'],
  components: {
  ...
  mounted () {
    this.makePic()
    this.qrcode = window.global.RequestBaseUrl + api.get_qrcode + '?alias=subscribe_company&url=' + location.href
  },
  methods: {
    makePic () {
      let that = this

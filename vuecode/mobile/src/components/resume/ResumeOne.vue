	...
	<div id="app" class="my_app">
		...
		<van-skeleton title avatar :row="10" :loading="mainLoading">
          ...
          <div class="invalid_resume" v-if="is_invalid===1">
            <img src="../../assets/images/resumeshow/invalid_resume.png">
					...
					<div class="up">
						...
						<div class="tx2">{{ base_info.age }}岁 · {{ base_info.experience_text }} · {{ base_info.education_text }}</div>
					...
					<div class="down">
						...
						<div class="right" @click="showDetail = !showDetail" v-if="moreDetailBtn">更多基本信息</div>
					</div>
					<div class="collect" @click="doFav">{{ has_fav == 1 ? '已收藏' : '收藏' }}</div>
					<div class="share" @click="doShare">分享</div>
				</div>
			</div>
			<div class="box_nav" v-if="$store.state.config.shortvideo_enable === '1'">
				<div class="item " :class="resumeShow === 'resume' ? 'active' : ''" @click="resumeShow = 'resume'">简历信息</div>
				<div class="item " :class="resumeShow === 'video' ? 'active' : ''" @click="resumeShow = 'video'">求职视频({{ videonum }})</div>
			</div>
			<div class="box_resume_some" v-if="resumeShow === 'resume'">
				...
				<div class="content_wrapper">
					<div class="box_3">
						...
						<div class="box_content">
							<div class="tx2" v-for="(item, index) in intention_list" :key="index">
								<div class="name">[{{ item.district_text }}] {{ item.category_text }}</div>
								<div class="intent">
					...
					<div class="box_cac">
						...
						<div class="box_content" v-if="show_contact == 1 && !phone_protect_open">
							...
							<div class="item wx" v-if="field_rule.contact.weixin != undefined && field_rule.contact.weixin.is_display == 1 && contact_info.weixin != ''">
								微信：{{ contact_info.weixin }}
							</div>
							<div class="item email" v-if="field_rule.contact.email != undefined && field_rule.contact.email.is_display == 1 && contact_info.email != ''">
								邮箱：{{ contact_info.email }}
							</div>
							<div class="item qq" v-if="field_rule.contact.qq != undefined && field_rule.contact.qq.is_display == 1 && contact_info.qq != ''">
								QQ：{{ contact_info.qq }}
							</div>
						</div>
				...
				<div class="content_wrapper">
					...
					<!--培训经历-->
					<div class="box_8" v-if="resume_module.training !== undefined && resume_module.training.is_display == 1 && training_list.length > 0">
					...
					<!--项目经历-->
					<div class="box_9" v-if="resume_module.project !== undefined && resume_module.project.is_display == 1 && project_list.length > 0">
          ...
          v-if="
              ...
              language_list.length > 0)
				...
				>
					<!--获得证书-->
					<div class="box_10" v-if="resume_module.certificate !== undefined && resume_module.certificate.is_display == 1 && certificate_list.length > 0">
						<div class="box_head"><div class="txt">获得证书</div></div>
						<div class="box_content">
							<div class="tx1" v-for="(item, index) in certificate_list" :key="index">
								{{ item.name }}
								<div class="right_txt">{{ item.obtaintime | monthTimeFilter }}</div>
							</div>
						</div>
					</div>
					<!--语言能力-->
					<div class="box_11" v-if="resume_module.language !== undefined && resume_module.language.is_display == 1 && language_list.length > 0">
						...
						<div class="box_content">
							<div class="tx1" v-for="(item, index) in language_list" :key="index">{{ item.language_text }}，{{ item.level_text }}</div>
						</div>
					</div>
				</div>
				<div class="form_split_10" v-if="resume_module.img !== undefined && resume_module.img.is_display == 1 && img_list.length > 0"></div>
        ...
        <div class="content_wrapper" v-if="enclosure_resume.enclosure">
					<div class="box_14">
						...
						<div class="box_content">
							<div class="resume_attachment" @click="attachmentDownload()" v-if="show_contact === 0">{{ enclosure_resume.title }}</div>
			...
			<div class="box_video_some" v-if="resumeShow === 'video'"><VideoList :videotype="2" :id="query_id" :gointype="'unitplay'"></VideoList></div>
		</van-skeleton>
		...
		>
			<div class="dialog_tip_wrapper">
				<div class="tx1" v-if="directServiceInfo.use_type == 'package'">很抱歉，您的简历点数不足，暂时无法下载简历建议您升级套餐或购买增值服务。</div>
				...
				<div class="tx2" v-if="parseInt(directServiceInfo.discount) > 0 && directServiceInfo.use_type != 'package'">
					...
					折，
					<router-link to="/member/order/add/common?type=service&service_type=resume_package" class="blue">立即了解</router-link>
				</div>
			</div>
		</van-dialog>
		<van-dialog
		...
		>
			<div class="dialog_tip_wrapper"><div class="tx1">很抱歉，您的套餐暂不支持下载简历，请升级套餐。</div></div>
		</van-dialog>
		...
		<van-dialog v-model="showDetail" title="更多基本信息">
			<div class="details_pop_box">
				<div class="detail_line" v-if="field_rule.basic.marriage !== undefined && field_rule.basic.marriage.is_display == 1 && base_info.marriage_text">
					<div class="line_left">{{ field_rule.basic.marriage.field_cn }}：</div>
					<div class="line_right">{{ base_info.marriage_text }}</div>
					<div class="clear"></div>
				</div>
				<div class="detail_line" v-if="field_rule.basic.height !== undefined && field_rule.basic.height.is_display == 1 && base_info.height != '' && base_info.height > 0">
					<div class="line_left">{{ field_rule.basic.height.field_cn }}：</div>
					<div class="line_right">{{ base_info.height }}CM</div>
					<div class="clear"></div>
				</div>
				<div class="detail_line" v-if="field_rule.basic.major !== undefined && field_rule.basic.major.is_display == 1 && base_info.major_text">
					<div class="line_left">{{ field_rule.basic.major.field_cn }}：</div>
					<div class="line_right">{{ base_info.major_text }}</div>
					<div class="clear"></div>
				</div>
				<div class="detail_line" v-if="field_rule.basic.householdaddress !== undefined && field_rule.basic.householdaddress.is_display == 1 && base_info.householdaddress">
					<div class="line_left">{{ field_rule.basic.householdaddress.field_cn }}：</div>
					<div class="line_right">{{ base_info.householdaddress }}</div>
					<div class="clear"></div>
				</div>
				<div class="detail_line" v-if="field_rule.basic.residence !== undefined && field_rule.basic.residence.is_display == 1 && base_info.residence">
					<div class="line_left">{{ field_rule.basic.residence.field_cn }}：</div>
					<div class="line_right">{{ base_info.residence }}</div>
					<div class="clear"></div>
				</div>
				<div class="detail_line" v-if="field_rule.basic.custom_field_1 !== undefined && field_rule.basic.custom_field_1.is_display == 1 && base_info.custom_field_1">
					<div class="line_left">{{ field_rule.basic.custom_field_1.field_cn }}：</div>
					<div class="line_right">{{ base_info.custom_field_1 }}</div>
					<div class="clear"></div>
				</div>
				<div class="detail_line" v-if="field_rule.basic.custom_field_2 !== undefined && field_rule.basic.custom_field_2.is_display == 1 && base_info.custom_field_2">
					<div class="line_left">{{ field_rule.basic.custom_field_2.field_cn }}：</div>
					<div class="line_right">{{ base_info.custom_field_2 }}</div>
					<div class="clear"></div>
				</div>
				<div class="detail_line" v-if="field_rule.basic.custom_field_3 !== undefined && field_rule.basic.custom_field_3.is_display == 1 && base_info.custom_field_3">
		...
		<van-popup :lazy-render="false" v-model="showInvite" position="right" :overlay="false" style="width:100%;height:100%">
			<AddInvitation ref="child" from="detail" :apply_fullname="base_info.fullname" :resume_id="base_info.id" @closePopup="showInvite = false"></AddInvitation>
		</van-popup>
		<van-popup :lazy-render="false" v-model="showTipoff" position="right" :overlay="false" style="width:100%;height:100%">
		...
		<van-popup v-model="showShare" position="bottom"><Share @cancelShare="cancelShare" @handleForward="handleForward" @handlePoster="handlePoster"></Share></van-popup>
		<div class="generate_posters_tpl1" @click="handlePoster">
			生成
			<br />
			海报
		</div>

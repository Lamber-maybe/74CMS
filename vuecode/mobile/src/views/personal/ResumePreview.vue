	...
	<div id="app">
			...
			<div class="box_2">
				<div class="name_group">
					<div class="name">
						...
						<div class="qu_ico" v-if="parseInt(basic.high_quality) === 1"></div>
						<div class="clear"></div>
					</div>
					<div class="tx1">{{ basic.current_text }}</div>
					<div class="info">{{ basic.age }}岁 · {{ basic.experience_text }} · {{ basic.education_text }}</div>
					<div :class="parseInt(basic.sex) === 1 ? 'avatar_box' : 'avatar_box fe'"><img :src="basic.photo_img_src" :alt="basic.fullname" /></div>
				</div>
				<div class="tx2">
		...
		<div class="content_wrapper">
			...
			<div class="box_3" v-if="intentionList.length">
				...
				<div class="box_content">
					<div class="tx2" v-for="(item, index) in intentionList" :key="index">
						...
						<div class="intent">
							{{ item.wage_text }}，{{ item.nature_text }} {{ fieldStore.intention.trade.is_display === 1 && item.trade_text !== '' ? `，${item.trade_text}` : '' }}
						</div>
					</div>
				</div>
			</div>
			...
			<div class="box_cac">
				...
				<div class="box_content">
					<div class="item phone" v-if="contact.mobile !== ''">手机：{{ contact.mobile }}</div>
					<div class="item wx" v-if="contact.weixin !== ''">微信：{{ contact.weixin }}</div>
					<div class="item email" v-if="contact.email !== ''">邮箱：{{ contact.email }}</div>
					<div class="item qq" v-if="contact.email !== ''">QQ：{{ contact.email }}</div>
				</div>
			</div>
			<!--自我描述-->
			<div class="box_5" v-if="basic.tag_text_arr.length || basic.specialty">
				...
				<div class="box_content">
					...
					<div class="tag" v-if="basic.tag_text_arr.length">
						<div class="tag-item" v-for="(item, index) in basic.tag_text_arr" :key="index">{{ item }}</div>
						<div class="clear"></div>
					</div>
					<div class="dec" v-if="basic.specialty"  style="white-space: pre-wrap">{{ basic.specialty }}</div>
				</div>
			</div>
			<!--教育经历-->
			<div class="box_6" v-if="educationList.length">
				...
				<div class="box_content">
					<div class="tx1" v-for="(item, index) in educationList" :key="index">
						...
						<div class="t2">{{ dateFormat(item.starttime) }} 至 {{ parseInt(item.todate) === 0 ? dateFormat(item.endtime) : '至今' }}</div>
			...
			<div class="box_10" v-if="certificateList.length">
				...
				<div class="box_content">
					<div class="tx1" v-for="(item, index) in certificateList" :key="index">
						{{ item.name }}
			...
			<div class="box_11" v-if="languageList.length">
				...
				<div class="box_content">
					<div class="tx1" v-for="(item, index) in languageList" :key="index">{{ item.language_text }}，{{ item.level_text }}</div>
				</div>
			</div>
			<!--我的作品-->
			<div class="box_12">
				...
				<div class="box_content">
					<div class="img_item" v-for="(item, index) in imgList" :key="index"><img :src="item.img_src" alt="item.title" class="img" @click="previewImg(index)" /></div>
				</div>
			</div>
			<!--附件简历-->
			<div class="box_12" v-if="enclosure_resume.enclosure">
			...
			<div class="box_13"><van-button round type="info" block native-type="button" @click="doShare">立即分享</van-button></div>
		</div>
		<van-dialog v-model="showDetail" title="更多基本信息">
			<div class="details_pop_box">
				...
				<div class="detail_line" v-if="fieldStore.basic.householdaddress.is_display">
					...
					<div class="line_right">{{ basic.householdaddress || '未填写' }}</div>
					<div class="clear"></div>
				</div>
				<div class="detail_line" v-if="fieldStore.basic.residence.is_display">
					...
					<div class="line_right">{{ basic.residence || '未填写' }}</div>
					<div class="clear"></div>
				</div>
				<div class="detail_line" v-if="fieldStore.basic.custom_field_1.is_display">
					...
					<div class="line_right">{{ basic.custom_field_1 || '未填写' }}</div>
					<div class="clear"></div>
				</div>
				<div class="detail_line" v-if="fieldStore.basic.custom_field_2.is_display">
					...
					<div class="line_right">{{ basic.custom_field_2 || '未填写' }}</div>
					<div class="clear"></div>
				</div>
				<div class="detail_line" v-if="fieldStore.basic.custom_field_3.is_display">
					...
					<div class="line_right">{{ basic.custom_field_3 || '未填写' }}</div>
					<div class="clear"></div>
				</div>
			</div>
		</van-dialog>
...
export default {
  ...
  methods: {
    ...
    dateFormatFull (time, isHM) {
      ...
      let timeText = ''
      if (isHM) {
        timeText = `${Y}-${this.supTen(M)}-${this.supTen(D)} ${this.supTen(H)}:${this.supTen(MM)}`
      } else {

...
class PdfService
{
    ...
    public function strToPdf($data = [])
    {
        ...
        $water_text = isset($data['water_text']) ? $data['water_text'] : '';//水印
        ...
        $mpdf->SetHTMLHeader('<p style="text-align:right;margin-right:35px;color:#0089FF;"><img style="width: 85px; height: 35px;" src="' . $title_header . '" /></p>');
        $mpdf->SetHTMLFooter('<p style="text-align:left;margin-left:35px;color:#777777;font-size: 12px">' . $title_footer . '</p>');
        $mpdf->WriteHTML($content);

        if ($is_down == 1) {
            //直接浏览器下载
            $mpdf->Output($file_name . '.pdf', 'F');
            return;
        } else {
            //浏览器输出
            $mpdf->Output();

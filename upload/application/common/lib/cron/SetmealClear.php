...
class SetmealClear
{
    ...
    {
        $timestamp = time();
        $uid_arr = \app\common\model\MemberSetmeal::where('deadline', '>', 0)
        ...
        \app\common\model\MemberSetmeal::where('uid', 'in', $uid_arr)
            ->update(['expired' => 1]);
        ...
        if (config('global_config.overtime_setmeal_jobnum') == 0) {
            ...
            $handler_uid_arr = [];
            $group_list = \app\common\model\JobSearchRtime::where('uid', 'in', $uid_arr)
            ...
            if (!empty($handler_uid_arr)) {
                ...
                foreach ($handler_uid_arr as $key => $value) {
                    $jobid_arr_tmp = \app\common\model\JobSearchRtime::where('uid', $key)
                        ->order('refreshtime asc,id asc')
                        ->limit($value)
                        ->column('id');
                    $jobid_arr = array_merge($jobid_arr, $jobid_arr_tmp);
                }
                if (!empty($jobid_arr)) {
                    \app\common\model\Job::whereIn('id', $jobid_arr)
                        ->setField('is_display', 0);

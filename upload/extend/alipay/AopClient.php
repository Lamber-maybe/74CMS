...
class AopClient
{
        ...
        if (is_array($postFields) && 0 < count($postFields)) {
            foreach ($postFields as $k => $v) {
                if ('@' != substr($v, 0, 1)) {
                    ...
                    $postBodyString .=
                        "$k=" .
                        urlencode($this->characet($v, $this->postCharset)) .
                        '&';
    ...
    protected function logCommunicationError(
        $apiName,
        $requestUrl,
        $errorCode,
        $responseTxt
    ...
    public function sdkExecute($request, $appAuthToken = null)
    {
        ...
        $params['version'] = $this->checkEmpty($version)
            ? $this->apiVersion
            : $version;
    ...
    public function pageExecute(
        ...
        if (
            method_exists($request, 'getNeedEncrypt') &&
            ...
            if (
                $this->checkEmpty($this->encryptKey) ||
                $this->checkEmpty($this->encryptType)
            ) {
                throw new Exception(
        ...
        //签名
        $totalParams['sign'] = $this->generateSign(
            $totalParams,
            $this->signType
        );
    ...
    public function execute(
        ...
        if ('json' == $this->format) {
            ...
            if (null !== $respObject) {
                $respWellFormed = true;
                $signData = $this->parserJSONSignData(
                    $request,
                    $resp,
                    $respObject
                );
    ...
    public function exec($paramsArray)
    {
        ...
        $requestClassName =
            ucfirst($inflector->camelize(substr($paramsArray['method'], 7))) .
            'Request';
    ...
    public function rsaCheckV1(
        ...
        return $this->verify(
            $this->getCheckSignContent($params),
            $sign,
            $rsaPublicKeyFilePath,
            $signType
        );
    ...
    function parserResponseSubCode(
        $request,
        $responseContent,
        $respObject,
        $format
        ...
        if ('json' == $format) {
            ...
            $rootNodeName =
                str_replace('.', '_', $apiName) . $this->RESPONSE_SUFFIX;
    ...
    function parserJSONSignData($request, $responseContent, $responseJSON)
    {
        ...
        $signData->signSourceData = $this->parserJSONSignSource(
            $request,
            $responseContent
        );
    ...
    function parserXMLSignData($request, $responseContent)
    {
        ...
        $signData->signSourceData = $this->parserXMLSignSource(
            $request,
            $responseContent
        );
    ...
    {
        if (
            !$this->checkEmpty($this->alipayPublicKey) ||
            ...
            // 获取结果sub_code
            $responseSubCode = $this->parserResponseSubCode(
                $request,
                $resp,
                $respObject,
                $this->format
    ...
    private function setupCharsets($request)
    {
        ...
        $str = preg_match('/[\x80-\xff]/', $this->appId)
            ? $this->appId
            : print_r($request, true);
        $this->fileCharset =
            mb_detect_encoding($str, 'UTF-8, GBK') == 'UTF-8' ? 'UTF-8' : 'GBK';
    }
    ...
    {
        $parsetItem = $this->parserEncryptJSONSignSource(
            $request,
            $responseContent
        );

        $bodyIndexContent = substr(
            $responseContent,
            0,
            $parsetItem->startIndex
        );
    ...
    private function parserEncryptJSONSignSource($request, $responseContent)
    {
        ...
        if ($rootIndex > 0) {
            return $this->parserEncryptJSONItem(
                $responseContent,
                $rootNodeName,
                $rootIndex
            );
    ...
    private function parserEncryptJSONItem(
        $responseContent,
        $nodeName,
        $nodeIndex
    ) {
    ...
    {
        $parsetItem = $this->parserEncryptXMLSignSource(
            $request,
            $responseContent
        );
    ...
    private function parserEncryptXMLSignSource($request, $responseContent)
    {
        ...
        if ($rootIndex > 0) {
            return $this->parserEncryptXMLItem(
                $responseContent,
                $rootNodeName,
                $rootIndex
            );
    ...
    private function parserEncryptXMLItem(
        $responseContent,
        $nodeName,
        $nodeIndex
    ) {
